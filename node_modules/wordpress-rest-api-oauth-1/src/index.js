...
export default class {
	...
	authorize(next) {
		var args = {};
		var savedCredentials = window.localStorage.getItem(
	...
	request(method, url, data = null) {
		...
		const requestUrls = [`${this.config.url}oauth1/request`, `${this.config.url}broker/connect`];

		/**
		 * Only attach the oauth headers if we have a request token, or it is a request to the `oauth/request` endpoint.
		 */
		if (
			(this.oauth && this.config.credentials.token) ||
			requestUrls.indexOf(url) > -1
		) {
		...
		return fetch(url, {
			...
			if (
				...
				response.headers.get('Content-Type').indexOf('x-www-form-urlencoded') >
					-1
			) {
